...
function jsonToGo(json, typename, flatten = true, allOmitempty = true)
{
	let data;
	let scope;
	let go = "";
	...
	typename = format(typename || "json2go-struct");
...
const convertJSON = (input, structName, inlineDefinitions, omitEmpty) => {
	...
	if (convertedOutput === "") {
		goStructTextArea.value = "error with input json";
		copyButton.disabled = true;
		return;
	};
	goStructTextArea.value = convertedOutput;
